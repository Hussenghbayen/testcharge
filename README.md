...
|---|---|---|
| _(اسم العضو 1)_ | Flutter / Frontend | _مثال: شاشات المصادقة، Router_ |
| _(اسم العضو 2)_ | Flutter / State Management | _مثال: Cubits، ApiClient_ |
| _(اسم العضو 3)_ | Backend | _مثال: API، قاعدة البيانات، صلاحيات الوصول_ |
| _(اسم العضو 4)_ | UI/UX + اختبارات | _مثال: Figma، الاختبارات الآلية_ |
...
- [Flutter](https://flutter.dev) 3.x / Dart — SDK: `^3.10.7`
- [flutter_bloc](https://pub.dev/packages/flutter_bloc) (Cubit) — إدارة الحالة
- [http](https://pub.dev/packages/http) — طبقة الاتصال بالشبكة (موحّدة عبر `ApiClient`)
- [firebase_auth](https://pub.dev/packages/firebase_auth) + [firebase_core](https://pub.dev/packages/firebase_core) — المصادقة
- [google_sign_in](https://pub.dev/packages/google_sign_in) — تسجيل دخول بحساب Google
- [flutter_dotenv](https://pub.dev/packages/flutter_dotenv) — إعدادات البيئة (`.env`)
- [equatable](https://pub.dev/packages/equatable) — مقارنة الحالات في Bloc
- [bloc_test](https://pub.dev/packages/bloc_test) + [mocktail](https://pub.dev/packages/mocktail) — اختبارات (dev)

**Backend (مستودع منفصل — راجع [روابط المشروع](#روابط-المشروع)):**
...
- [Flutter SDK](https://docs.flutter.dev/get-started/install) (قناة stable، تتوافق مع Dart `^3.10.7`)
...
- اتصال بمستودع الباك اند (شغّال أو محلي) — راجع [روابط المشروع](#روابط-المشروع)
...
3. **Android:** أضف تطبيق Android بنفس `applicationId` الموجود في
   `android/app/build.gradle.kts`، وحمّل `google-services.json` وضعه في `android/app/`
4. **iOS:** أضف تطبيق iOS بنفس Bundle ID الموجود في `ios/Runner.xcodeproj`، وحمّل
   `GoogleService-Info.plist` وضعه في `ios/Runner/`
5. من إعدادات المشروع (Project Settings → General) انسخ **Web API Key** وضعه في
...
APK الإصدار المستقر يُرفع تحت [GitHub Releases](../../releases) لهذا المستودع.
...
| العنصر | الرابط |
|---|---|
| مستودع الباك اند | ⚠️ _أضيفوا الرابط هون_ |
| الباك اند (بيئة التشغيل) | `https://charging-api-tkne.onrender.com` |
| تقرير التخرج (PDF/Doc) | ⚠️ _أضيفوا الرابط هون_ |
| ملف Figma | ⚠️ _أضيفوا الرابط هون_ |
